...
###############################
options = find_hyper_params_svm(X, y)
# make a dataframe
df_options = pd.DataFrame(options)
# select best hyperparameters
k = df_options.iloc[df_options[0].idxmax()][1]
g = df_options.iloc[df_options[0].idxmax()][2]

print("SVM, Kernel=", k, ", gamma=", g)
...
svm = SVC(kernel = k, gamma = g, probability = True, random_state=1)
...
for train_index, test_index in rskf.split(X.T, y['status']):
    ...
    probas_ = svm.fit(X_train, y_train).predict_proba(X_train)
...
plt.savefig("figures/roc_SVM.png")
...
svm.fit(X.T.values, y['status'].values)
...
probas_ = svm.predict_proba(X_validation_standard)
